...
AUTH0_DOMAIN = 'dev-ru7nim8q.us.auth0.com'
ALGORITHMS = ['RS256']
...
def get_token_auth_header():
    ...
    if len(auth_arr_parts) != 2:
        raise AuthError({
            'AUthentication header should contain only two part "Bearer" and JWT.'
        }, 401)

    return auth_arr_parts[1]
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'Token payload should include permissions'
        }, 400)
    if permission not in payload['permissions']:
        raise AuthError({
            'No Permission for this user available'
        }, 403)
